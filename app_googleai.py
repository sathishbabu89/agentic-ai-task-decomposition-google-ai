...
from google.cloud import aiplatform  # Google Cloud AI Platform library

# Function to initialize the Gemini model (Gemini 2.0 Flash) with Google API Key
def get_google_llm():
    ...
    aiplatform.init(
        project=project_id,
        location=location,
        credentials=api_key  # Pass API Key directly (or use Service Account if needed)
    )
    
    # Create the LLM object configured for Gemini 2.0 Flash model
    llm = LLM(
        model="gemini-2.0-flash",  # The specific model you want to use
        api_key=api_key,
        base_url="https://generativelanguage.googleapis.com/v1beta/models",  # Google AI endpoint
        temperature=float(os.getenv("GOOGLE_TEMPERATURE", "0.7")),
    )
    
    return llm

# Configure the LLM (Google Gemini 2.0 Flash)
llm = get_google_llm()
